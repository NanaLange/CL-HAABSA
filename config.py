...
tf.app.flags.DEFINE_integer('num_buckets', 6, 'number of buckets for curriculum learning')

# traindata, testdata and embeddings, train path aangepast met ELMo
tf.app.flags.DEFINE_string("train_path_ont", "data/programGeneratedData/GloVetraindata"+str(FLAGS.year)+"cross_train_10.txt", "train data path for ont")
tf.app.flags.DEFINE_string("test_path_ont", "data/programGeneratedData/GloVetestdata"+str(FLAGS.year)+"cross_train_10.txt", "formatted test data path")
...
tf.app.flags.DEFINE_string("remaining_svm_test_path", "data/programGeneratedData/"+str(FLAGS.embedding_dim)+'remainingsvmtestdata'+str(FLAGS.year)+"cross_train_10.txt", "formatted remaining test data path after ontology")
...
tf.app.flags.DEFINE_string("hyper_train_path_ont", "data/programGeneratedData/"+str(FLAGS.embedding_dim)+'hypertraindataGloVe'+str(FLAGS.year)+".txt", "hyper GloVe train data path")
tf.app.flags.DEFINE_string("hyper_eval_path_ont", "data/programGeneratedData/"+str(FLAGS.embedding_dim)+'hyperevaldataGloVe'+str(FLAGS.year)+".txt", "hyper GloVe eval data path")
...
# obtain the aspect categories
tf.app.flags.DEFINE_string("train_aspect_categories", "data/programGeneratedData/train_aspect_categories"+str(FLAGS.year)+"cross_train_10.txt", "the aspect categories of the train data")
tf.app.flags.DEFINE_string("test_aspect_categories", "data/programGeneratedData/test_aspect_categories"+str(FLAGS.year)+"cross_train_10.txt", "the aspect categories of the test data")

tf.app.flags.DEFINE_string("hyper_train_aspect_categories", "data/programGeneratedData/hyper_train_aspect_categories"+str(FLAGS.year)+"cross_train_10.txt", "the aspect categories of the hyper train data")
tf.app.flags.DEFINE_string("hyper_eval_aspect_categories", "data/programGeneratedData/hyper_eval_aspect_categories"+str(FLAGS.year)+"cross_train_10.txt", "the aspect categories of the hyper eval data")

# obtain the curriculum scores data
tf.app.flags.DEFINE_string("sorted_indices", "data/programGeneratedData/curriculumLearning/sortedindices"+str(FLAGS.year)+".csv", "the indices sorted by curriculum learning")
tf.app.flags.DEFINE_string("bucket_traindata", "data/programGeneratedData/curriculumLearning/curriculumhypertraindata"+str(FLAGS.year)+"_"+str(FLAGS.num_buckets)+"_", "the hyper train data (80%)")
tf.app.flags.DEFINE_string("bucket_evaldata", "data/programGeneratedData/curriculumLearning/curriculumhypereval"+str(FLAGS.year)+"_"+str(FLAGS.num_buckets)+"_", "the hyper evaluation data (20%)")
