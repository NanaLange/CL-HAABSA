...
with open('BERT_Large_2016embedding.txt') as BERTemb:
    with open("data/programGeneratedData/" + str(FLAGS.embedding_type) + '_'
              + str(FLAGS.year)+ '.' + str(FLAGS.embedding_dim) + 'cross_train_10.txt','w') as outfile:
        for line in BERTemb:
            word =  line.split(" ")[0]
            counter +=1
            print(counter)
...
for filenr in range (1,8):
    ...
    sentCount = 0
    dollarcount = 0
    with open('data/temporaryData/unique2016_BERT_Data_' + str(filenr) + 'cross_train_10.txt','w') as outFile:
