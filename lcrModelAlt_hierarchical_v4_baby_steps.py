...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, sort, num_buckets,
    ...
    with tf.Session(config=config) as sess:
        ...
        # divide the first bucket in 80% train and 20% validation set
        data1 = buckets[0]
        np.random.shuffle(data1)
        tmp = int(round(0.8 * len(data1)))
        tr_data1 = data1[:tmp]
        ev_data1 = data1[tmp:]
        bucket_number = 1
        ...
        for bucket in buckets:  # for every bucket of data
            ...
            if bucket_number > 1:  # add new bucket to train (80%) and validation (20%) set
                np.random.shuffle(bucket)
                tmp = int(round(0.8 * len(bucket)))
                train = bucket[:tmp]
                evals = bucket[tmp:]
                traindata = np.concatenate((traindata, np.asarray(train)))
                evaldata = np.concatenate((evaldata, np.asarray(evals)))
