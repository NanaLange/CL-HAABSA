...
def getWeightsPerSentence(i):
    outfname = 'data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/elmo' + str(FLAGS.year) + 'weight_' + str(i / 3) + 'cross_train_10.txt'
...
filenames=['data/programGeneratedData/300traindata' + str(FLAGS.year) + 'cross_train_10.txt',
           'data/programGeneratedData/300testdata' + str(FLAGS.year) + 'cross_train_10.txt']
with open('data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/raw_data' + str(FLAGS.year) + 'cross_train_10.txt','w') as outfile:
    for fname in filenames:
        with open(fname) as infile:
            for line in infile:
                outfile.write(line)
lines = open('data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/raw_data' + str(FLAGS.year) + 'cross_train_10.txt').readlines()
...
with open('data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/elmoAllVocab'+ str(FLAGS.year) +'cross_train_10.txt','w') as outFile:
    for i in range(0, len(lines), 3):
         print("sentence: " + str(i / 3) + " out of " + str(len(lines) / 3) + " in " + "raw_data;")
...
lines = open('data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/raw_data' + str(FLAGS.year) + 'cross_train_10.txt').readlines()
vocab_old = open('data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/elmoAllVocab'+ str(FLAGS.year) +'cross_train_10.txt').readlines()
...
with open('data/programGeneratedData/' + str(FLAGS.embedding_dim) + 'traindata'+ str(FLAGS.year) +'cross_train_10.txt','w') as outTrain,\
        open('data/programGeneratedData/' + str(FLAGS.embedding_dim) + 'testdata'+ str(FLAGS.year) +'cross_train_10.txt','w') as outTest:
...
with open('data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/elmoAllVocab'+ str(FLAGS.year) +'cross_train_10.txt') as ev:
  with open('data/temporaryData/temp_ELMo'+ str(FLAGS.year) +'/ELMo' + str(FLAGS.year) + 'weights.txt') as ew:
    with open("data/programGeneratedData/"+str(FLAGS.embedding_dim)+'ELMo_wordEmbedding'+ str(FLAGS.year) +'cross_train_10.txt',"w") as ep:
