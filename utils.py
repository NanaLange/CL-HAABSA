...
def curr_batch_index(bucket, batch_size, is_shuffle=True):
    if is_shuffle:
        np.random.shuffle(bucket)
    for i in range(int(len(bucket)/batch_size) + (1 if len(bucket) % batch_size else 0)):
        yield bucket[i * batch_size:(i + 1) * batch_size]
...
def change_y_to_onehot(y):
    ...
    print(Counter(y))
    class_set = set(y)
    if '0' not in class_set: # needed for curriculum learning, since some sets might not contain 0/1/-1.
        class_set.add('0')
    if '-1' not in class_set:
        class_set.add('-1')
    if '1' not in class_set:
        class_set.add('1')
    print(class_set)
